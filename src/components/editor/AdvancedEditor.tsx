...
const AdvancedEditor: React.FC = () => {
  ...
  const presetStyles = [
    ...
    {
      name: 'Vibrant',
      filters: { brightness: 115, contrast: 125, saturate: 150 }
    },
    {
      name: 'Soft Focus',
    ...
    {
      name: 'Cool Tone',
      filters: { brightness: 105, contrast: 110, saturate: 120 }
    },
    {
      name: 'Warm Tone',
      filters: { brightness: 110, contrast: 105, saturate: 115 }
    }
  ];
  ...
  const handleDownload = async () => {
    if (!user) {
      toast.error('Please sign up to download your design.');
      setTimeout(() => {
      window.location.href = '/auth';
      }, 1500); // ✅ redirect to signup/login page
      return;
    }
          ...
          <div className="p-6">
                  ...
                  {adjustmentControls.map((control) => (
                    <div key={control.key}>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-xs text-gray-400">
                          {adjustments[control.key as keyof typeof adjustments]}{control.unit}
                        </span>
                      </div>
                      <input
                        type="range"
                        min={control.min}
                        max={control.max}
                        value={adjustments[control.key as keyof typeof adjustments]}
                        onChange={(e) => {
            ...
            {activeTab === 'ai' && (
              <div className="space-y-6">
                    ...
                    <input
                      ...
                      onKeyPress={(e: React.KeyboardEvent<HTMLInputElement>) => {
                        if (e.key === 'Enter' && (e.target as HTMLInputElement).value.trim()) {
                          handleAIEdit((e.target as HTMLInputElement).value);
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => handleAIEdit('copy designs and reapply to projects')}
                    disabled={isProcessing}
                    className="w-full bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 disabled:opacity-50 text-white py-3 px-4 rounded-lg font-medium transition-colors"
                  >
                    Copy & Reapply Design
                  </button>
                  <button
                    onClick={() => handleAIEdit('suggest algorithmic improvements')}
                    disabled={isProcessing}
                    className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 disabled:opacity-50 text-white py-3 px-4 rounded-lg font-medium transition-colors"
                  >
