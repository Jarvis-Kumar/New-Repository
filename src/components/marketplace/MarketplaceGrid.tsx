...
const MarketplaceGrid: React.FC = () => {
  ...
  const { favoritePresets, toggleFavorite } = useStore();
  ...
  const handlePurchase = async (preset: any) => {
    if (preset.isPremium) {
  ...
  const PresetCard = ({ preset }: { preset: any }) => {
    ...
    const { trigger: triggerPurchase, AuthModalComponent: PurchaseAuthModal } = useAuthAction(() => handlePurchase(preset));
    const { trigger: triggerFavorite, AuthModalComponent: FavoriteAuthModal } = useAuthAction(() => toggleFavorite(preset.id));

    return (
      <>
        <motion.div
          whileHover={{ y: -5 }}
          className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl overflow-hidden hover:border-primary-500/50 transition-all duration-300 group h-full flex flex-col"
        >
          ...
          <div className="p-4 flex-grow flex flex-col">
            ...
            {/* Platforms */}
            <div className="flex flex-wrap gap-1 mb-4">
              {preset.platforms.map((platform: string) => (
                <span
                  key={platform}
            ...
            <button
              ...
              className="w-full bg-primary-600 hover:bg-primary-700 text-white py-2 px-4 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2 mt-auto"
            >
              {preset.isPremium ? (
                <>
  ...
  <Link to={`/dataset/${preset.id}`} key={preset.id}>
    <motion.div
